...
import openDeploy from '@/graphql/OpenDeploy.gql';
...
const myProjects = createComponent({
    setup() {
        ...
        openDeploy
        const {mutate: open} = useMutation(openDeploy);
        const selectProject = async (item: any) => {
            // @ts-ignore
            const send = await open({ uuid:  +item.uuid ? +item.uuid :0 })
	        console.log(send)
            location.href = '/task'
        }
        return {
